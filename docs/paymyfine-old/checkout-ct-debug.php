...
if (isset($_POST['process']) && $_POST['process'] == 'payment' && $valid) {
	...
	try {
		...
		if ($success || $debug) {
			...
			foreach ($citationInfo['Citations'] as $citation) {
				...
				if ($success || $debug) {
					...
					try {
						...
						if (!$debug) {
							$statement->execute(array(
								...
								':court_date' => $citation['CourtDate'],
								':charges' => $citation['Charges'],
								':fine_amount' => $citation['FineAmount'],
								':total' => number_format(($data['total'] / 100), 2),
								':ws_response' => serialize($response),
								':receipt_number' => $receiptNumber
							));
					...
					$courtSubject = 'Confirmation of Citation Payment';
					$message2court='
						    <table width="650" >
			  <tr>
			    <td align="left" valign="middle"><table width="100%" >
			      <tr>
				<td>&nbsp;</td>
				<td align="center" valign="middle"><h3>Citation Payment Details</h3> </td>
				<td>&nbsp;</td>
			      </tr>
			      <tr>
				<td>&nbsp;</td>
		...
		if ($success || $debug) {
			...
			$customerSubject = 'Personal Receipt - Citation Payment Details';
		  ...
		  <tr>
		    <td align="left" valign="middle"><table width="100%" >
		      <tr>
			<td>&nbsp;</td>
			<td align="center" valign="middle"><h3>Citation Payment Details</h3>
			  </td>
			<td>&nbsp;</td>
		      </tr>
		      <tr>
		...
		if (!$success && !$debug) {
			...
			header("Location: confirmation-ct.php");
		}
	}
	catch (Exception $e) {
		echo $e->getMessage();
...
try {
	$citationsToPay = $db->query("select * from citation_cart_temp where ct_session_id='$sessionId'");
} catch (PDOException $e) {
	echo $e->getMessage();
}
$citationCount = $citationsToPay->rowCount();

// Setup all the citations that the user wants to pay
while ($cartRow = $citationsToPay->fetch(PDO::FETCH_BOTH)) {
	// Select the citation information
	$citationId = $cartRow['product_id'];
	try {
		$citationResult = $db->query("select * from citation_test where id='$citationId'");
	} catch (PDOException $e) {
		echo $e->getMessage();
	}
	$citationRow = $citationResult->fetch(PDO::FETCH_BOTH);
