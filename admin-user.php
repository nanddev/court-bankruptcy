...
function genRandomString() {
  ...
  $string ='CourtPaymentsDemo';    
    for ($p = 1; $p < 3; $p++) {
        //$string .= $characters[mt_rand(0, strlen($characters))];
		        //$string .= $characters[mt_rand(0, strlen($characters))];
				$value= $characters[mt_rand(0, strlen($characters))];
				if(strlen($value<=1))
				{
				$string .= "0".$value;
				}
...
if(isset($_REQUEST['Save']))
{
 ...
 {
  header("location:admin-user.php?msg=2");
  }
  else
  {
   ...
   $insert_admin=mysql_query("insert into administrator(name,username,password,email,user_type,date) values ('$name','$random','$password','$email','$user_type','$todaydate')");
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" >
  <tr  style="color:#000;" align="center">
	...
	if($_REQUEST['msg']=='1')
	{
	echo "Admin User Details added success";
	}
	if($_REQUEST['msg']=='2')
	{
    ...
    <td><form name="admin_formreg" id="admin_formreg" method="post" action="">
      <table width="100%" border="0" cellspacing="3" cellpadding="3">
        ...
        <tr>
          <td>User Type</td>
          <td>Super
              <input name="user_type" type="radio" value="Super" />
&nbsp;&nbsp; View Only
      <input name="user_type" type="radio" value="View Only" checked="checked" />
          </td>
        </tr>
        <tr>
          <td colspan="2" align="center"><input name="Save" type="submit" id="Save" onclick="return admin_register_validate();" value="Save" /></td>
        </tr>
      </table>
    </form></td>
    <td>&nbsp;</td>
